"""
game_ui.py
可视化界面 - 完整版（支持联机+AI对战）/ Visual Interface - Complete Version (Network + AI Support)

优化内容 / Optimizations:
1. 简略日志缓存系统，解决性能问题
2. 改进简略日志解析，显示完整动作和状态
3. 无回合数限制的永久缓存
"""
...
try:
    ...
    AI_AVAILABLE = False
    print("[Warning] ai_player.py not found, AI mode disabled")
...
class GameUI:
    def __init__(self):
        ...
        # 网络模式
        self.network_mode = None  # None(本地), 'host', 'client', 'ai_p1', 'ai_p2'
        self.network_server = None
        self.network_client = None
        self.my_player_id = None  # 1 or 2，表示我控制哪个玩家
        
        # AI模式
        self.ai_player = None  # AI对象
        ...
        #  简略日志缓存系统
        self.simple_log_cache = {}  # {turn: [formatted_lines]}
    ...
    def init_ai(self, ai_is_p2=True):
        """初始化AI / Initialize AI"""
        if not AI_AVAILABLE:
            print("[Error] AI模块不可用，无法启动AI模式")
            self.game_phase = "menu"
            return
        
        if ai_is_p2:
            self.ai_player = AIPlayer(self.player2, self.player1, self.combat)
            print("[AI] AI控制 Blue (P2)")
        else:
            self.ai_player = AIPlayer(self.player1, self.player2, self.combat)
            print("[AI] AI控制 Red (P1)")
    ...
    def draw_menu(self):
        ...
        if self.menu_input_mode == 'ip_input':
            ...
            if AI_AVAILABLE:
                options.extend([
                    ("2 - 人机对战 (你是P1)", "2 - VS AI (You are P1)", (WINDOW_WIDTH//2, menu_y + 70)),
                    ("3 - 人机对战 (你是P2)", "3 - VS AI (You are P2)", (WINDOW_WIDTH//2, menu_y + 140)),
                ])
                network_y = menu_y + 210
            else:
                network_y = menu_y + 70
            
            options.extend([
                ("4 - 创建房间（我是P1）", "4 - Host Room (I am P1)", (WINDOW_WIDTH//2, network_y)),
                ("5 - 加入房间（我是P2）", "5 - Join Room (I am P2)", (WINDOW_WIDTH//2, network_y + 70)),
                ("", "ESC - Exit", (WINDOW_WIDTH//2, network_y + 170))
            ])
        ...
        if self.menu_input_mode == 'ip_input':
            ...
            if event.key == pygame.K_1:
                ...
                if not AI_AVAILABLE:
                    print("[Error] AI模块不可用")
                    return
                self.network_mode = 'ai_p1'
                self.my_player_id = 1
                self.init_game()
                self.init_ai(ai_is_p2=True)
                self.game_phase = "game"
                print("人机对战: 你是P1 (Red), AI是P2 (Blue)")
                ...
                if not AI_AVAILABLE:
                    print("[Error] AI模块不可用")
                    return
                self.network_mode = 'ai_p2'
                self.my_player_id = 2
                self.init_game()
                self.init_ai(ai_is_p2=False)
                self.game_phase = "game"
                print("人机对战: AI是P1 (Red), 你是P2 (Blue)")
    ...
    def _cache_simple_log(self, turn):
        """解析并缓存当前回合的简略日志 / Parse and cache current turn's simple log"""
        if turn in self.simple_log_cache:
            return
        
        messages = self.combat.get_turn_log(turn)
        if not messages:
            return
        
        formatted = self._parse_and_format_simple_log(messages)
        self.simple_log_cache[turn] = formatted
        print(f"[Cache] 已缓存回合{turn}的简略日志 / Cached turn {turn} simple log")
    
    def _parse_and_format_simple_log(self, messages):
        """解析消息并格式化为简略日志 / Parse messages and format as simple log"""
        # 提取回合信息和帧数据
        parsed = self._parse_frame_info(messages)
        
        # 格式化输出
        return self._format_simple_log(parsed)
    
    def _parse_frame_info(self, messages):
        """核心解析：从消息中提取结构化数据 / Core parsing: extract structured data from messages"""
        result = {
            'turn_info': None,
            'frames': {1: {'p1': {}, 'p2': {}}, 2: {'p1': {}, 'p2': {}}}
        }
        ...
        for msg in messages:
            ...
            # 提取回合信息
            if "回合" in clean and "距离:" in clean:
                result['turn_info'] = clean
                continue
            
            # 识别帧
            if "--- 第" in clean and "帧" in clean:
                if "第 1 帧" in clean:
                    current_frame = 1
                elif "第 2 帧" in clean:
                    current_frame = 2
            ...
            # 解析每条消息
            self._parse_single_message(clean, result['frames'][current_frame], p1_name, p2_name)
        
        return result
    
    def _parse_single_message(self, msg, frame_data, p1_name, p2_name):
        """解析单条消息，填充frame_data / Parse single message, fill frame_data"""
        ...
        # 动作声明
        if "尝试攻击" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['action'] = '攻击'
            if "基础范围" in msg:
                # 提取范围和伤害
                if "范围1格" in msg:
                    frame_data[player]['action_detail'] = ''
        
        elif "尝试蓄力" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            if "蓄力1" in msg:
                frame_data[player]['action'] = '蓄力1'
            elif "蓄力2" in msg:
                frame_data[player]['action'] = '蓄力2'
        
        elif "尝试控制" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['action'] = '控制'
        
        elif "防御姿态" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['action'] = '防御'
        
        elif "防御反击姿态" in msg or "反击姿态" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['action'] = '反击'
        
        elif "使用抱摔" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['action'] = '抱摔'
        
        elif "使用投掷" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['action'] = '投掷'
        
        elif "爆血" in msg and "爆血生效" not in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['action'] = '爆血'
        
        elif "尝试冲刺" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['action'] = '冲刺'
        
        # 结果
        elif "攻击命中" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            if 'result' not in frame_data[player]:
                frame_data[player]['result'] = '命中'
            # 提取伤害
            if "伤害" in msg:
                try:
                    damage = msg.split("伤害")[1].split(")")[0].strip()
                    frame_data[player]['result'] = f'命中{damage}伤'
                except:
                    pass
        
        elif "攻击未命中" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['result'] = '未命中'
        
        elif "控制成功" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['result'] = '成功'
            ...
            player = 'p1' if p1_name in msg else 'p2'
            frame_data[player]['result'] = '失败'
        
        elif "闪避成功" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            if 'result' not in frame_data[player]:
                frame_data[player]['result'] = ''
            frame_data[player]['dodge'] = True
        ...
        # 位置变化
            ...
            try:
                parts = msg.split("->")
                if len(parts) >= 2:
                    end_pos = parts[1].split()[0].split('(')[0].strip()
                    frame_data[player]['pos'] = end_pos
                    if 'action' not in frame_data[player]:
                        frame_data[player]['action'] = f'移动到{end_pos}格'
            except:
                pass
        ...
        # Buff变化
            ...
            player = 'p1' if p1_name in msg else 'p2'
            if "蓄力1" in msg:
                if 'buffs' not in frame_data[player]:
                    frame_data[player]['buffs'] = []
                frame_data[player]['buffs'].append('蓄力1')
                if 'result' not in frame_data[player]:
                    frame_data[player]['result'] = '成功'
            elif "蓄力2" in msg:
                if 'buffs' not in frame_data[player]:
                    frame_data[player]['buffs'] = []
                frame_data[player]['buffs'].append('蓄力2')
                if 'result' not in frame_data[player]:
                    frame_data[player]['result'] = '成功'
        
        elif "消耗蓄力" in msg or "蓄力1加成" in msg or "蓄力2加成" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            if 'buffs' not in frame_data[player]:
                frame_data[player]['buffs'] = []
            if '蓄力消耗' not in frame_data[player]['buffs']:
                frame_data[player]['buffs'].append('蓄力消耗')
        
        elif "获得冲刺buff" in msg or "将获得冲刺buff" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            if 'buffs' not in frame_data[player]:
                frame_data[player]['buffs'] = []
            frame_data[player]['buffs'].append('冲刺+1')
        
        elif "消耗冲刺buff" in msg or "将消耗冲刺buff" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            if 'buffs' not in frame_data[player]:
                frame_data[player]['buffs'] = []
            frame_data[player]['buffs'].append('冲刺-1')
        
        elif "冲锋加成" in msg or "冲刺buff" in msg:
            player = 'p1' if p1_name in msg else 'p2'
            if 'buffs' not in frame_data[player]:
                frame_data[player]['buffs'] = []
            # 标记使用了buff
            if '使用冲刺' not in frame_data[player].get('buffs', []):
                frame_data[player]['buffs'].append('使用冲刺')
    
    def _format_simple_log(self, parsed):
        """格式化简略日志输出 / Format simple log output"""
        lines = []
        
        # 回合信息
        if parsed['turn_info']:
            lines.append(parsed['turn_info'])
            lines.append("")
        
        # 遍历两帧
        for frame_num in [1, 2]:
            frame = parsed['frames'][frame_num]
            
            # 检查是否有内容
            has_content = frame['p1'].get('action') or frame['p2'].get('action')
            if not has_content:
                continue
            
            lines.append(f"帧{frame_num} / Frame {frame_num}")
            
            # P1
            if frame['p1'].get('action'):
                line = f"  {PLAYER1_NAME}: {self._format_player_action(frame['p1'])}"
                lines.append(line)
            
            # P2
            if frame['p2'].get('action'):
                line = f"  {PLAYER2_NAME}: {self._format_player_action(frame['p2'])}"
                lines.append(line)
            
            lines.append("")
        
        return lines if lines else ["(无信息) / (No information)"]
    
    def _format_player_action(self, data):
        """格式化单个玩家的动作信息 / Format single player's action info"""
        parts = []
        ...
        if action_detail:
            parts.append(f"{action}({action_detail})")
        else:
            parts.append(action)
        ...
        status = []
        if data.get('hp'):
            status.append(f"HP:{data['hp']}")
        if data.get('pos'):
            status.append(f"Pos:{data['pos']}")
        
        # Buff变化
        buffs = data.get('buffs', [])
        if buffs:
            status.append(f"[{','.join(buffs)}]")
    ...
    def _extract_simple_log(self, messages, turn=None):
        """提取简略日志（优先使用缓存）/ Extract simple log (prioritize cache)"""
        #  优先从缓存获取
        if turn is not None and turn in self.simple_log_cache:
            return self.simple_log_cache[turn]
        
        # 未缓存，执行解析
        return self._parse_and_format_simple_log(messages)
    ...
    def draw_player(self, player, color):
        ...
        pygame.draw.rect(self.screen, RED, (hx, hy, HP_BAR_WIDTH, HP_BAR_HEIGHT))
    ...
    def draw_key_hints(self):
        ...
        current_frame_idx = len(current_actions)
    ...
    def draw_messages(self):
        ...
        if self.simple_log_mode:
            #  传入回合号以使用缓存
            current_turn = self.viewing_turn if self.is_viewing_history and self.viewing_turn > 0 else self.combat.turn
            messages = self._extract_simple_log(messages, current_turn)
    ...
    def draw_selection(self):
        ...
        if self.network_mode in ['client', 'ai_p2']:
            if not self.p1_locked:
                p1_text = f"{PLAYER1_NAME}: [???] [???]"
            else:
                p1_text = f"{PLAYER1_NAME}: [已确认] / [Confirmed]"
            bg = LIGHT_RED
        elif self.network_mode in ['host', 'ai_p1'] and self.current_player == 2:
            if not self.p1_locked:
                p1_text = f"{PLAYER1_NAME}: [???] [???]"
            else:
                p1_text = f"{PLAYER1_NAME}: [已确认] / [Confirmed]"
        ...
        if p1_status and self.network_mode not in ['client', 'ai_p2']:
        ...
        if self.network_mode in ['host', 'ai_p1']:
            if not self.p2_locked:
                p2_text = f"{PLAYER2_NAME}: [???] [???]"
            else:
                p2_text = f"{PLAYER2_NAME}: [已确认] / [Confirmed]"
            bg = LIGHT_BLUE
        elif self.network_mode in ['client', 'ai_p2'] and self.current_player == 1:
            if not self.p2_locked:
                p2_text = f"{PLAYER2_NAME}: [???] [???]"
            else:
                p2_text = f"{PLAYER2_NAME}: [已确认] / [Confirmed]"
        ...
        if p2_status and self.network_mode not in ['host', 'ai_p1']:
    ...
    def _get_hint(self):
        """获取提示信息"""
        if self.network_mode in ['ai_p1', 'ai_p2']:
            if self.p1_locked and self.p2_locked:
                return ("双方已确认，执行中... / Both confirmed, executing...", ORANGE)
            
            if self.network_mode == 'ai_p1':
                player = self.player1
                actions = self.p1_actions
            ...
            for i in range(2):
                if i >= len(actions):
                    if player.is_frame_locked(next_turn, i+1):
                        ...
                        return (f"{player_name} 第{i+1}帧 / {player_name} Frame {i+1}", GREEN)
            
            return ("已选完，按 SPACE 确认 / Selection complete, press SPACE to confirm", ORANGE)
    ...
    def handle_input(self, event):
        ...
        # AI模式特殊处理
        if self.network_mode in ['ai_p1', 'ai_p2']:
            if self.current_player != self.my_player_id:
                return
        ...
        if self.network_mode == 'ai_p1':
            ...
            if not self.p2_locked:
                ...
                if ready:
                    ...
                    # AI自动选择
                    self.p1_actions = self.ai_player.choose_turn_actions()
                    self._finalize_actions(self.player1, self.p1_actions, next_turn)
                    self.p1_locked = True
                    print(f"[AI] {PLAYER1_NAME}已选择动作")
    ...
    def execute_frame(self):
        ...
        try:
            ...
            if idx == 1:
                ...
                self._cache_simple_log(self.combat.turn)
                self.log_scroll_offset = 9999
        finally:
    ...
    def reset_turn(self):
        ...
        if self.network_mode in ['host', 'ai_p1']:
            self.current_player = 1
        elif self.network_mode in ['client', 'ai_p2']:
            self.current_player = 2
        else:
            self.current_player = 1
