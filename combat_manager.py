"""
combat_manager.py
战斗管理器 - 7阶段状态化重构版（日志优化版） / Combat Manager - 7-Phase State-based Refactored Version (Log Optimized)
...
class CombatManager:
    ...
    def _stage1_apply_actions(self, p1_act, p2_act, frame):
        ...
        # 爆血（特殊：需要预计算距离） / Burst (special: needs to pre-calculate distance)
        if p1_act == 'burst':
    ...
    def _check_dodge(self, mover, attacker, mover_act, attacker_act):
        ...
        # 移动前能打到，但伤害被移除了（说明移动后打不到） / Could hit before move, but damage removed (means cannot hit after move)
    ...
    def _stage6_settle_all(self, p1_act, p2_act, frame):
        ...
        # 6.1 结算标记 / Settle markers
        self._settle_markers()
        
        # 6.2 结算Buff（前置：蓄力） / Settle buffs (pre: charge)
        self._settle_buffs_pre()
        
        # 6.3 结算控制（pull和controlled） / Settle control (pull and controlled)
        self._settle_control()
        
        # 6.4 结算位置 / Settle positions
        self._settle_positions(p1_act, p2_act)
        
        # 6.5 结算伤害 / Settle damage
        self._settle_damage()
        
        # 6.6 后处理：蓄力2硬直 / Post-processing: Charge 2 stun
        self._post_check_charge_2_stun(frame)
        
        # 6.7 结算Buff（后置：冲刺） / Settle buffs (post: dash)
        self._settle_buffs_post()
    
    def _settle_markers(self):
        """结算标记状态 / Settle marker states"""
    ...
    def _settle_damage(self):
        ...
        # ===== 生成buff消耗状态（在阶段6.7结算） / Generate buff consumption states (settled in phase 6.7) =====
    ...
    def _stage7_release_adjustment(self, current_frame):
        ...
        # 帧结束自动解除控制（只在"下一帧"才解除） / Auto release control at frame end (only release on "next frame")
        ...
        # 技能触发的解除（grab/throw/burst） / Release triggered by skills (grab/throw/burst)
    ...
    def _handle_skill_release(self, p1_has_release, p2_has_release):
        """处理技能触发的控制解除 / Handle control release triggered by skills"""
        distance = self.get_distance()
    ...
    def _predict_final_positions(self):
        """预计算最终位置 / Pre-calculate final positions"""
