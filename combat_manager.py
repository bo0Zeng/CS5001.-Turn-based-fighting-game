"""
combat_manager.py
战斗管理器 - 7阶段状态化重构版（完全重构）
Combat Manager - 7-Phase State-based Refactored Version (Complete Refactor)

重构要点：
1. 阶段1：只为自身生成do_xxx状态
2. 阶段2：自身状态冲突检测
3. 阶段3：验证有效性，为对方添加be_xxx状态和实际伤害状态
   - 在这里处理蓄力加成、冲刺加成
   - 在这里计算最终伤害值
4. 阶段4-7：保持原有逻辑
"""
...
class CombatManager:
    ...
    def _validate_counter(self, player, opponent):
        ...
        # 检查是否受到非自伤的攻击
        has_dmg = any(s.source not in ["burst_self", "charge_penalty", "charge_interrupt"] 
                     for s in player.damage_states)
        ...
        if has_dmg:
            ...
            opponent.add_action_state('took_damage')
    ...
    def _validate_and_apply_burst(self, attacker, defender, distance):
        ...
        attacker.add_action_state('took_damage')
